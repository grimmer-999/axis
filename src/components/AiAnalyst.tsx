...
/* ═══════════════════════════════════════════════════════════════
   AXIS — AI Intelligence Analyst Panel
...
export default function AiAnalyst({ data }: AiAnalystProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('axis-gemini-key');
    if (saved) {
  ...
  const getHeaders = useCallback((): Record<string, string> => {
    ...
    const savedKey = localStorage.getItem('axis-gemini-key');
    if (savedKey) {
  ...
  const saveApiKey = useCallback(() => {
    ...
    if (key) {
      localStorage.setItem('axis-gemini-key', key);
      setKeySaved(true);
  ...
  const clearApiKey = useCallback(() => {
    localStorage.removeItem('axis-gemini-key');
    setApiKeyInput('');
    setKeySaved(false);
  }, []);
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="hud-text text-[11px] text-[var(--text-heading)]">AXIS ANALYST</span>
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-[8px] font-mono text-[var(--text-muted)] leading-relaxed">
                        Your key is stored locally and sent only to the AXIS server. Get a free key at{' '}
                        <a
